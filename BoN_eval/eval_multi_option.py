...
exper2path = {
    ...
    # ],
    'all': [
        os.path.join(root_path, 'deepseek-coder-33b-base_deepseek-coder-33b-base/lr1e-5'),
        os.path.join(root_path, 'pmp_rm1_lm1_7b_1b_14M_wsd_deepseek-coder-33b-base-lr1e-5/lr1e-5'),
    ]
}
...
for key, paths in exper2path.items():
    paths = [path.format(root_path=root_path, scaling_path=scaling_path) for path in paths if not (os.path.exists(os.path.join(path, 'test', 'MO_logiqa2_test_acc_no_aggregation.json'))and os.path.exists(os.path.join(path, 'test', 'MO_reclor2_test_acc_no_aggregation.json')))]
...
def process_path(path, rank):
    try:
        ...
        # 运行Shell脚本
        subprocess.run(['bash', '/root_path/CodePMP/BoN_eval/RM_rank_inference+eval_reclor_NR_multi_option_for14b.sh', path, str(rank)], check=True)
        subprocess.run(['bash', '/root_path/CodePMP/BoN_eval/RM_rank_inference+eval_logiqa2_NR_multi_option_for14b.sh', path, str(rank)], check=True)
