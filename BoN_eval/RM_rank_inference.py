...
PROMPT_DICT = {
    ...
    "prompt_codeultrafeedback": (
        "### Instruction:\nWrite an executable code according to the following requirements.\n\n{query}\n\n### Response:"
    ),
}
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        sources = [("\n".join(item['input'].split('\n')[1:]).strip() + item['output']).replace("### ", "") for item in dataset_for_eval]
        labels = [item['gold_answer'].replace("### ", "") for item in dataset_for_eval]
        data_dict = preprocess(sources, tokenizer)

        self.input_ids = data_dict["input_ids"] + data_dict["input_ids"][-100:]
        self.labels = labels + labels[-100:]
        self.ids = [item['id'] for item in dataset_for_eval]
...
def main(args):
    ...
    data_path = os.path.join(args.out_path, '{}_test_rank_{}_generated_by_{}_{}.json'.format(fp_name.split('_')[0], '_'.join(fp_name.split('_')[2:5]), data_path.split('/')[-1], process_index))
