...
def main(root_path, dataset_name, generator_name, BoN=256):
    ...
    fps = [fp for fp in os.listdir(root_path) if f'MO_{dataset_name}_test_rank' in fp]
    qid2data = {}
    ...
    json.dump(data, open(os.path.join(root_path, 'MO_{}_test_{}.json'.format(dataset_name, args.aggregation_function)), 'w'))
    json.dump({'acc': acc}, open(os.path.join(root_path, 'MO_{}_test_acc_{}.json'.format(dataset_name, args.aggregation_function)), 'w'))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--BoN", default=-1, type=int, help="model path")
